"""LightGBM LambdaRank trainer/predictor.

This module expects parquet files containing:
  - customer_id, article_id
  - label (required for train/valid)
  - feature columns (tabular + embedding/CF if available)

It groups by customer_id for ranking.
"""
...
def _group_sizes(df: pd.DataFrame, group_col: str = "customer_id") -> np.ndarray:
    sizes = df.groupby(group_col, sort=False).size().to_numpy(dtype=np.int32)
    return sizes
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train/predict LightGBM LambdaRank.")
